...
theme/
  ...
  shared/keyboard.css   on-screen keyboard: glass sheet, glass keys, accent states      (done)
...
7. ~~The on-screen keyboard.~~ Done (2026-09-12), not yet reviewed by Zoe: `shared/keyboard.css`.
   ...
   `everywhere` rather than only in SP. The keyboard themes itself from a block of CSS variables on
   the Keyboard element, one set per built-in skin (`.DefaultTheme`, `.Pumpkin`, `.Grape`...), so
   redefining the DefaultTheme set recolours nearly all of it in one place. Two catches:
   DefaultTheme **hardcodes its pressed colour** (Steam blue) instead of reading
   `--key-touched-background-color`, so that state needs a real rule; and the keyboard already
   draws its own `backgroundglass` layer sized to its own box (not the whole screen), which is
   what gets rounded and inset to become the floating sheet. Checked: the d-pad still walks the
   keys, focus / latched / one-shot / pressed states, the emoji and Steam-items panel, and Reduce
   transparency. **Not** checked: over a running game, and a keyboard opened from a Quick Access
   text field (no reachable input to test with).
